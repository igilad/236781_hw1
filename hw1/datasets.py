...
def random_labelled_image(
    ...
    label = int(torch.randint(0, num_classes-1, (1,)))
    # ========================
    return image, label
...
class RandomImageDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> Tuple[Tensor, int]:
        ...
        with torch_temporary_seed(index):
            X,y = random_labelled_image(self.image_dim, self.num_classes, 0, 256)
            return (X,y)
...
class ImageStreamDataset(IterableDataset):
    ...
    def __iter__(self) -> Iterator[Tuple[Tensor, int]]:
        ...
        n = 0
        while True:
            ds = RandomImageDataset(n+1, self.num_classes, *self.image_dim)
            image = ds[n]
            yield image
            n += 1
        # ========================
